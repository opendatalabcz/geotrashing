...
class DBConn:
    def __init__(self, db_host: str, db_name: str, db_user: str, db_password: str, db_debug: bool = False):
        """
        Toolkit for DB requests
        :param db_host: Host name of the DB ("db" for docker)
        :param db_name: Name of the DB
        :param db_user: User of the DB
        :param db_password: User password
        :param db_debug: If true, the request are simulated.
        """
    ...
    def get_device_id(self,
                      ...
                      cpu: str):
        """
        Get id of device given mainly by its fingerprint
        :param fingerprint: Fingerprint generated by the FingerprintJS
        :param ip: IP of the device
        :param browser: Type of the browser
        :param os: Os of the device
        :param hw: Device name
        :param cpu: Cpu Type of device
    ...
    def get_bin_desc(self, bin_id):
        """
        Get bin description given its ID
        :param bin_id:
        :return: String description of the bin
        """
    ...
    def insert_bin(self, bin_id: int, latlng: list, desc: str, address_id: int):
        """
        Insert new bin to the DB
        :param bin_id: its id
        :param latlng: its coordinates
        :param desc: its description
        :param address_id: its address ID
    ...
    def get_latlng_status(self, latlng: list):
        """
        Get information about fullness within given coordinates
        :param latlng: coordinates of bins
        :return: Column names and status in the neighborhood
        """
        if not isinstance(latlng, list):
            raise re.RequestError
        if not len(latlng) == 2:
    ...
    def get_bins_latlng(self):
        """
        Get all bins with its coordinates
        :return:
        """
        if self._db_debug_mode:
    ...
    def get_requests(self, bin_ids, from_time, to_time):
        """
        Get all requests within given time for given bins
        :param bin_ids:
        :param from_time:
        :param to_time:
        :return:
    ...
    def set_request(self, bin_id, device_id, min_delay_minutes, fill_perc):
        """
        Add new request at given bin
        :param bin_id:
        :param device_id:
        :param min_delay_minutes: Ignore new request at this bin id for given time
        :param fill_perc: Fullness of the bin
        :return: None
        """
        if not isinstance(bin_id, int):
    ...
    def _with_cursor(self, function):
        """
        Utility creating connector and cursor to the DB and execute the given function
        :param function: Function querying DB using connection to DB
    ...
    def _with_connection(self, function):
        """
        Utility creating connection to the DB and execute the given function
        :param function: Function querying DB using connection to DB
        :return: Result of the Query
        """
        conn = None
    ...
    def _get_device_id(cur,
                       ...
                       cpu: str):
        """
        Get device id given its description
        :param cur: Cursor the database
        :param fingerprint: Fingerprint generated by FingerprintJS
        :param ip: device ip
        :param browser: device browser
        :param os: device os
        :param hw: device hw
        :param cpu: device cpu
        :return: device ID
        """
    ...
    def _get_bin_desc(cur, bin_id):
        """
        Get description of the bin given by id
        :param cur: Cursor the database
        :param bin_id:
    ...
    def _get_requests(self, bin_ids, from_time, to_time, min_perc):
        """
        Get all request at bins within given time and minimal fullness
        :param bin_ids: List of bin ids
        :param from_time:
        :param to_time:
        :param min_perc: Minimal fullness of the request
        :return: Requests
        """
        select_requests = '''
    ...
    def _get_latlng_status(cur, lat, lon, last_hours=8):
        ...
        :param lat: latitude
        :param lon: longitude
        :param last_hours: since
        :return: overall status of the bins
        """
        select_latlng_agg = '''
            SELECT  bin_id as id,
